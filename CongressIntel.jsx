  ...
  return (
    <div>
      ...
      <div style={{ marginTop:"32px", padding:"14px 18px", background:T.goldFaint,
        ...
        ⚠ DISCLAIMER: Analysis generated by AI using illustrative data for educational purposes only.
  ...
  const runQuickScan = async () => {
    ...
    try {
      const text = await callClaude(
        "You are a U.S. congressional trading intelligence analyst. Generate realistic illustrative data for educational use. Return ONLY a valid JSON array, no markdown.",
        `Generate exactly 10 realistic illustrative U.S. congressional stock trades from the past 30 days. Vary parties, chambers, sectors, risk levels. Include 2+ high-risk trades (score 7.5+). Return JSON array, each object:
{"id":<1-10>,"member":"<Full Name>","party":"<R|D>","chamber":"<House|Senate>","state":"<2-letter>","committee":"<committee>","ticker":"<TICKER>","company":"<Company>","sector":"<sector>","type":"<Buy|Sell>","amount":"<$X,000 - $XX,000>","disclosedDate":"<YYYY-MM-DD>","daysLate":<1-45>,"score":<1.0-10.0>,"scoreReason":"<one sentence>"}
  ...
  const runDeepAnalysis = async () => {
    ...
    for (const trade of sel) {
      ...
      for (let i = 0; i < prompts.length; i++) {
        setPipelineStep(i+1);
        try { sr[STEPS[i].key] = await callClaude(prompts[i][0], prompts[i][1]); }
